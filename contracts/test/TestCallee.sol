...
contract TestCallee is ISwapCallback {
    ...
    function swapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external override {
        ...
        if (amount0Delta > 0) {
            IERC20(IPool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(amount0Delta));
        } else {
            assert(amount1Delta > 0);
            IERC20(IPool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(amount1Delta));
        }
    }
}
